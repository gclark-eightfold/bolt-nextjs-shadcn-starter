...
function SortableOverlay(props: SortableOverlayProps & { withPortalFix?: boolean }) {
  const { dropAnimation: dropAnimationProp, children, withPortalFix, ...overlayProps } = props;
  const context = React.useContext(SortableRoot);
  if (!context) {
  ...
  return withPortalFix ? createPortal(overlay, document.body) : overlay;
}
